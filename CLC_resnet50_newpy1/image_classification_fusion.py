...
from tensorflow.keras.layers import (
    Input, Dense, Concatenate, Add, ReLU, BatchNormalization,
    Multiply, Dropout, Conv2D, MaxPooling2D,
...
def engineer_features(row_values):
    ...
    for (a, b, c) in ANGLE_TRIPLETS:
        ...
        feats.append(angle_at_b(ax, ay, bx, by, cx, cy) / 180.0)
    ...
    for tip_idx in [12, 16, 20, 24, 28]:
        ...
        feats.append(euclidean(tx, ty, *r_root))

    for tip_idx in [33, 37, 41, 45, 49]:
        tx, ty = get_xy(kps, tip_idx)
        feats.append(euclidean(tx, ty, *l_root))
...
def build_image_path(row):
    ...
    return os.path.join(IMAGE_ROOT, word_name, video_folder, image_name)
...
drop_cols = [
    "image_path", "skeletal_image_path", "new_image_path",
    "video_name", "video_number", "word_name", "label"
]
...
X_pose = np.vstack([
    ...
    for row in df[coord_cols].replace(-1, 0).values
])
...
train_mask = np.isin(video_nums, [1,2,3,4,5,6,7])
val_mask   = np.isin(video_nums, [8])
...
X_pose_val   = scaler.transform(X_pose_val)
...
plt.tight_layout()
plt.savefig(
    os.path.join(PLOT_SAVE_DIR, "feature_correlation_matrix.png"),
    dpi=150
)
plt.close()
print(f"\nCorrelation plot saved to: {PLOT_SAVE_DIR}")
...
print("="*60)
print("\nNumber of features before removal:", X_pose_train.shape[1])
print("Features removed:", len(to_drop))
print("Feature indices removed:", to_drop)

X_pose_train = df_train_corr.drop(columns=to_drop).values
X_pose_val   = pd.DataFrame(X_pose_val).drop(columns=to_drop).values
X_pose_test  = pd.DataFrame(X_pose_test).drop(columns=to_drop).values

print("\nNumber of features after removal:", X_pose_train.shape[1])
print("\nNew train shape:", X_pose_train.shape)
print("\nNew val shape:", X_pose_val.shape)
print("\nNew test shape:", X_pose_test.shape)
...
def parse_function(img_path, pose_feat, label):
    ...
    return (
        {"image_input": img, "pose_input": pose_feat},
        label
    )
...
# Optional:
# Freeze backbone initially for comparison
...
cnn_proj = Dense(1024)(cnn_features)
cnn_proj = BatchNormalization()(cnn_proj)

cnn_proj = Dense(512)(cnn_proj)
cnn_proj = Dropout(0.4)(cnn_proj)

cnn_proj = Dense(256)(cnn_proj)
cnn_proj = BatchNormalization()(cnn_proj)

cnn_proj = Dense(128)(cnn_proj)
cnn_proj = ReLU()(cnn_proj)

# =========================================================
# POSE BRANCH  (unchanged)
# =========================================================

pose_input = Input(shape=(X_pose_train.shape[1],), name="pose_input")

pose_branch = Dense(1024)(pose_input)
...
pose_branch = Add()([pose_branch, pose_residual])
pose_branch = ReLU()(pose_branch)

# =========================================================
# GATED FUSION  (unchanged, dim-matched to cnn_proj=128)
# =========================================================

cnn_gate  = Dense(128, activation='sigmoid')(cnn_proj)
pose_gate = Dense(256, activation='sigmoid')(pose_branch)

gated_cnn  = Multiply()([cnn_proj,    cnn_gate])    # 128-d
gated_pose = Multiply()([pose_branch, pose_gate])   # 256-d

combined = Concatenate()([gated_cnn, gated_pose])   # 384-d

# =========================================================
# CLASSIFIER HEAD  (one Dense block instead of two)
# =========================================================
...
with open(RESULT_LOG_PATH, "a", encoding="utf-8") as f:
    ...
    best_msg = (
        ...
        f"Training Accuracy at Best Epoch: {best_train_acc:.4f}\n"
        f"Training Loss at Best Epoch: {best_train_loss:.4f}\n"
